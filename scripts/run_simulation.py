...
save_video = True  # If False, view the planner as it's running (note this may be slow due to overhead from plotting)
...
x_min, x_max = (-2, 42)
y_min, y_max = (0, 10)
start_pose = np.array([0., 5., 0.])  # x, y, orientation
...
w = params.robot_width * 3.  # no trees will generate within this distance of start and goal
invalid_spaces = np.array([[start_pose[0], start_pose[1], w],
                           [goal_position[0], goal_position[1], w]])
bounds = [x_min, x_max, y_min, y_max]
forest = PoissonForestUniformRadius(bounds=bounds, density=forest_density,
                                    radius_min=min_tree_radius, radius_max=max_tree_radius,
                                    seed=forest_seed, invalid_spaces=invalid_spaces)

# Initialize the simulation
# Using multiple hypothesis planner
if not use_baseline:
    sim = MultipleHypothesisPlannerSimulation(start_pose=start_pose, goal_position=goal_position,
                                              params=params, world=forest,
                                              range_bearing_sensor=rb_sensor, size_sensor=size_sensor)
# Using baseline
else:
