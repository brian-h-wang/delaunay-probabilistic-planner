...
def covariance_ellipse(mean, cov, ax, n_std=3.0, extra_radius=0.0, facecolor='none',
                       ellipse: patches.Ellipse = None, **kwargs):
    """
...
class PlotRobot(Plotter):
    ...
    def __init__(self, ax, position, orientation=0.0, width=1.0, zorder=0, color=(0.4, 0.4, 0.8),
                 linewidth=1.0):
        super().__init__(ax)
        self.radius = width/2
        ...
        rect_height = width / 4.
        rect_width = (width/2.) * 1.4  # how much the line should stick out the front of the robot
        rect_xy = position
        R = np.array([[math.cos(orientation), math.sin(orientation)],
                      [-math.sin(orientation), math.cos(orientation)]])
        rect_xy = position + (R.dot(np.array([0, -rect_height/2.]).reshape((2,1)))).flatten()
        ...
        self.line_handle, = ax.plot(xs, ys, '-', linewidth=linewidth, color=self.dark_blue,
    ...
    def set_pose(self, position, orientation):
        ...
        R = np.array([[math.cos(orientation), math.sin(orientation)],
                      [-math.sin(orientation), math.cos(orientation)]])
        rect_xy = position + (R.dot(np.array([0, -self.rect_height/2.]).reshape((2,1)))).flatten()
...
class ObstaclePlotter(Plotter):
    ...
    def __init__(self, obstacle: Obstacle2D, ax: plt.Axes = None, color=None,
                 label: Optional[str] = None):
        super().__init__(ax)

        ## Plot the mean
        position_mean = obstacle.pos_mean
        radius_mean = obstacle.size_mean / 2.  # diameter to radius
        if color is None:
        ...
        if label is not None:
            label_handle = ax.text(position_mean[0], position_mean[1], str(label),
                                   ha='center', va='center', c=(0.8,0.8,0.9),
                                   zorder=10)
        else:
            label_handle = None
...
class ObstacleEstimatePlotter(ObstaclePlotter):
    ...
    def __init__(self, obstacle: Obstacle2D, n_std_devs: int = 2, ax: plt.Axes = None,
                 label=None, color=None, show_size_uncertainty=True):
        """
        ...
        radius_mean = obstacle.size_mean / 2.  # diameter to radius
        radius_std = math.sqrt(obstacle.size_var) / 2.
        ...
        if n_std_devs > 0 and show_size_uncertainty:
            circle_small = patches.Circle(position_mean, radius=radius_mean - n_std_devs * radius_std,
                                          edgecolor=self.med_light_blue, fill=False,
                                          linestyle='--', zorder=5)
            circle_large = patches.Circle(position_mean, radius=radius_mean + n_std_devs * radius_std,
                                          edgecolor=self.med_light_blue, facecolor=self.light_blue,
                                          linestyle='--', zorder=3)
    ...
    def update(self, obstacle: Obstacle2D, color=None):
        ...
        if self.circle_small is not None and self.circle_large is not None:
            for circle, r in zip([self.circle_small, self.circle_large],
                                 [radius_mean - self.n_std_devs * radius_std,
                                  radius_mean + self.n_std_devs * radius_std]):
        ...
        if self.ellipse_xy is not None:
            position_cov = obstacle.pos_cov
            self.ellipse_xy = covariance_ellipse(position_mean, position_cov, self.ax,
                                                 ellipse=self.ellipse_xy,
                                                 n_std=self.n_std_devs)
...
class CellDecompositionPlotter(Plotter):
    ...
    def __init__(self, cell_decomposition: SafetyCellDecomposition,
                 gray: bool = False,
                 show_labels: bool = False, show_uncertainty: bool = False,
                 ax: plt.Axes = None,
                 show_colorbar=False):
        super().__init__(ax)
        ...
        if show_colorbar:
            ...
            colorbar = plt.colorbar(cm.ScalarMappable(cmap=self.cmap), cax=cax,
                                         label="Cell edge safety probability",
                                    ticks=[0, 1])
    ...
    def _add_obstacle(self, obstacle, obs_idx, range_zone: int = RANGE_SHORT):
        ...
        if self._show_uncertainty:
            self.obstacle_plotters[obs_idx] = ObstacleEstimatePlotter(obstacle, ax=self.ax,
                                                                      label=obs_label,
                                                                      color=obs_color)
        else:
            self.obstacle_plotters[obs_idx] = ObstaclePlotter(obstacle, ax=self.ax,
                                                              label=obs_label, color=obs_color)

    def _add_cell_label(self, cell_decomposition, cell_idx):
        vertices = cell_decomposition.get_cell_vertices(cell_idx)
        position = np.mean(vertices, axis=0)
        cell_label_handle = self.ax.text(position[0], position[1], str(cell_idx),
                                    ha='center', va='center', c=(0.6,0.6,0.9),
                                    zorder=10)
...
class DistanceGraphPlotter(Plotter):
    ...
    def __init__(self, distance_graph: NavigationGraph, ax=None, vertex_excluded: VertexPropertyMap=None,
        ...
        if vertex_excluded is not None:
            ...
            exc_x = []
            exc_y = []
        excluded_handle, = self.ax.plot(exc_x, exc_y, 'x', c=x_color, markersize=x_size, zorder=6,
                                        markeredgewidth=2)
        ...
        if show_start_and_goal:
            ...
            start_handle, = self.ax.plot(start_position[0], start_position[1], '^',
                    c=start_color, markersize=cell_vertex_size, zorder=11, )
            goal_handle, = self.ax.plot(goal_position[0], goal_position[1], '*',
                    c=goal_color, markersize=cell_vertex_size, zorder=11)
        else:
            start_handle = None
            goal_handle = None
    ...
    def _get_line_segments(self, distance_graph: NavigationGraph):
        ...
        for e in graph.edges():
            if not self.show_start_and_goal:
                s = int(e.source())
                t = int(e.target())
                if (s == distance_graph.start_vertex or t == distance_graph.start_vertex or
                    s == distance_graph.goal_vertex or t == distance_graph.goal_vertex):
                    continue
...
class LocalPlannerPathPlotter(Plotter):
    ...
    def plot_waypoints(self, waypoints):
        if self.waypoints_handle is None:
            self.waypoints_handle = self.ax.plot(waypoints[:, 0], waypoints[:, 1], '-', linewidth=1.5,
                                                 color=self.med_blue, zorder=10,
                                                 markersize=2.)[0]
        else:
...
class BaselineHighLevelPathPlotter(Plotter):
    ...
    def __init__(self, points: ArrayLike, ax=None):
        ...
        lw = 2.0
        self.path_handle, = self.ax.plot([], '.-', lw=lw, markersize=4,
                                        color=Plotter.med_light_blue,
                                        label="2D global planner path",
                                        zorder=5)
...
class HighLevelPathPlotter(Plotter):
    ...
    def __init__(self, mhp_result: MultipleHypothesisPlanningResult, ax=None,
                 cmap='plasma', n_paths: int = 1, show_cell_decomposition=True,
                 colors=None, lw: Optional[float]=None, no_legend=False):
        super().__init__(ax=ax)
        ...
        if show_cell_decomposition:
            self.cell_decomposition_plotter = CellDecompositionPlotter(mhp_result.cell_decomposition,
        ...
        for path_idx in range(n_paths):
            ...
            self.path_handles += ax.plot([], lw=linewidths[path_idx],
    ...
    def update(self, mhp_result: MultipleHypothesisPlanningResult):
        ...
        for path_idx in range(self.n_paths):
            ...
            if path_idx >= len(paths):
                ...
                label = self._get_label(path_idx, dist_cost=distance_costs[path_idx],
                                        safety_cost=safety_costs[path_idx])
...
class BloatedObstaclePlotter(Plotter):
    ...
    def update(self, obstacles):
        for i in range(obstacles.shape[0]):
            ...
            radius = obstacles[i, 2] / 2.
            if i >= len(self.circles):
                circle = patches.Circle(position, radius=radius, color=self.light_green,
                                        alpha=0.3, zorder=3)
...
class SinglePathPlotter(Plotter):
    ...
    def __init__(self, mhp_result: MultipleHypothesisPlanningResult, path_idx: int,
                 ax=None, cmap='plasma', show_cell_decomposition=True,
                 colors=None):
        super().__init__(ax=ax)
        ...
        self.path_handle, = ax.plot(xy[:,0], xy[:,1], lw=linewidth, color=color,
...
class GroundTruthSubplot(SubplotInterface):
    ...
    def __init__(self, ax: plt.Axes, xlim: Tuple[float, float], ylim: Tuple[float, float], sim: Simulation, title=None):
        ...
        self.plot_robot = PlotRobot(self.ax, position=sim.robot_position,
                             width=robot_width, color=self.robot_color)

        # Plot the obstacles
        for i in range(sim.n_obstacles):
            p = patches.Circle(sim.obstacle_positions[i,:], radius=sim.obstacle_diameters[i] / 2,
...
class SLAMSubplot(SubplotInterface):
    ...
    def __init__(self, ax: plt.Axes, xlim: Tuple[float, float], ylim: Tuple[float, float], sim: Simulation, title=None):
        ...
        self.plot_robot = PlotRobot(self.ax, position=sim.robot_position, width=robot_width,
                                    color=self.robot_color, zorder=100)
        self.ax_slam_obstacles = {}
        line_segments = detections_to_lines(sim.detections_xy_global)
        self.ax_slam_detections = LineCollection(line_segments, linewidths=1.2,
                                                 colors=Plotter.light_green, zorder=8)
    ...
    def update(self, sim: Simulation):
        ...
        self.pose_cov_handle = covariance_ellipse(pose, pose_cov, self.ax, n_std=2.0,
        ...
        for obs_idx, b_obstacle in enumerate(boundary_obstacles):
            b_obs_idx = -(obs_idx + 1) # give boundary obstacles negative indices to avoid conflict with the regular obstacles
            handle: Optional[ObstacleEstimatePlotter] = \
                self.ax_slam_obstacles.setdefault(b_obs_idx, None)
            if handle is None:
...
class BaselinePlannerSubplot(SubplotInterface):
    ...
    def update(self, sim: Simulation):
        ...
        obstacles = sim.slam.get_landmarks_estimate()
        for obs_idx, obstacle in enumerate(obstacles):
            handle: Optional[ObstaclePlotter] = \
                self.baseline_obstacles.setdefault(obs_idx, None)
            if handle is None:
...
class CellDecompositionSubplot(SubplotInterface):
    ...
    def __init__(self, ax: plt.Axes, xlim: Tuple[float, float], ylim: Tuple[float, float], sim: Simulation, title=None,
                 plot_search_tree=False, plot_bloated=False):
        super().__init__(ax=ax, xlim=xlim, ylim=ylim, sim=sim, title=title)
    ...
    def update(self, sim: Simulation):
        if sim.cell_decomposition is not None:
            ...
            if self.cell_decomp_plot is None:
                cell_decomp_plot = CellDecompositionPlotter(cell_decomp, ax=self.ax,
                                                            show_labels=False,
        ...
        if self.plot_bloated:
            if self.bloated_obstacles is None:
                self.bloated_obstacles = BloatedObstaclePlotter(sim.bloated_obstacles,
                                                                       ax=self.ax)
            else:
    ...
    def update(self, sim: Simulation):
        if sim.mhp_result is not None:
            if self.path_plotter is None:
                show_cd = True
                self.path_plotter = HighLevelPathDistancePlotter(sim.mhp_result,
                                                                         ax=self.ax,
...
class SimulationPlotter(object):
    ...
    def __init__(self, sim: Simulation, plot_bounds, fig=None, interactive=False,
                 minimal=False, plot_search_tree=False, plot_bloated=False, dpi=None):
        """
...
class SimulationAnimator(object):
    ...
    def __init__(self, plotter, max_time=None):
        ...
        self.anim = FuncAnimation(plotter.fig, self.animate, interval=interval,
                                  save_count=n_frames_cache, repeat=False)
        plt.draw()
        plt.show()
